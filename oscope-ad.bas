...
REPEAT
 ...
 ADSTOP
 FOR w=1 TO width
 data(w)=(BSTR(MID$(h$,w*2-1,2),2) & 0x3FF)*height/1024
 NEXT w
...
dpage: code=WEBOUT(wc,0,"<h1>Oscilloscope "+STR$(cn)+"</h1>"+br$)
 code=WEBOUT(wc,0,"<META HTTP-EQUIV=""Refresh"" CONTENT=5>")
 code=WEBOUT(wc,0,"<table border=0><tr><td>3.3V</td><td></td><td></td></tr><tr><td></td><td><img src=""/basr/x.bmp"" width="+STR$(width)+" height="+STR$(height)+"></td><td></td></tr>")
