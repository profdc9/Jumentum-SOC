 ...
 * @{
 */
static uint32_t setSSPclock (LPC_SSP_TypeDef *SSPx, uint32_t target_clock);
 ...
 * @return 		None
 *********************************************************************/
uint32_t SSP_Init(LPC_SSP_TypeDef *SSPx, SSP_CFG_Type *SSP_ConfigStruct)
{
	uint32_t tmp;
	...
	return setSSPclock(SSPx, SSP_ConfigStruct->ClockRate);
}
