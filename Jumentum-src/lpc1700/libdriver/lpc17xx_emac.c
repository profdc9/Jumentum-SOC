...
#define EMACMEM 0x20081500


typedef struct _emac_space
{
/** Rx Descriptor data array */
 RX_Desc e_Rx_Desc[EMAC_NUM_RX_FRAG];
/** Rx Status data array - Must be 8-Byte aligned */
 __attribute__ ((aligned (8))) RX_Stat e_Rx_Stat[EMAC_NUM_RX_FRAG];
/** Tx Descriptor data array */
 TX_Desc e_Tx_Desc[EMAC_NUM_TX_FRAG];
/** Tx Status data array */
 __attribute__ ((aligned (8))) TX_Stat e_Tx_Stat[EMAC_NUM_TX_FRAG];
/** Rx buffer data */
   uint32_t e_rx_buf[EMAC_NUM_RX_FRAG][EMAC_ETH_MAX_FLEN>>2];
/** Tx buffer data */
   uint32_t e_tx_buf[EMAC_NUM_TX_FRAG][EMAC_ETH_MAX_FLEN>>2];
} emac_space;

#define EMAC_SPACE ((emac_space *)EMACMEM)

/* EMAC local DMA buffers */

#define rx_buf (EMAC_SPACE)->e_rx_buf
#define tx_buf (EMAC_SPACE)->e_tx_buf
#define Rx_Desc (EMAC_SPACE)->e_Rx_Desc
#define Rx_Stat (EMAC_SPACE)->e_Rx_Stat
#define Tx_Desc (EMAC_SPACE)->e_Tx_Desc
#define Tx_Stat (EMAC_SPACE)->e_Tx_Stat
/**
 * @}
 */
...
Status EMAC_Init(EMAC_CFG_Type *EMAC_ConfigStruct)
{
	...
	for (clr=(unsigned int *)EMAC_SPACE;clr<(((unsigned char *)EMAC_SPACE)+sizeof(*EMAC_SPACE));) 
		*clr++ =0;
