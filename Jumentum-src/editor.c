...
C play macro, record;       D cursor right, line end;\n\r\
E cursor up, file top;      F forward search, backward search;\n\r\
G goto line, goto column;   H delete before, delete under;\n\r\
I tab, tab insert;          K block copy, block cut;\n\r\
L refresh screen, status;   N toggle insert mode;\n\r\
...
W save and/or exit;         X cursor down, file bottom;\n\r\
Y block delete line;\n\r\
\n\
Each letter represent a control key. Each key corresponds one or two\n\r\
editing commands (separated by comma). Press the control key for the\n\r\
...
ESTATIC int x;
ESTATIC int sww = 78;                   /* screen position 1 <= x <= sww */
...
ESTATIC int get_key()
{
  ...
  if(key == 27) {
    getch();  /* aa nothing */
    if((s=strchr(k1, inchar())) != NULL && (key = k2[s-k1]) > 'a')
      flag[ALT]++;
    return key&0x1F;
  }
  return key;
...
ESTATIC void show_status()
{
  char tbuf[80];
  tbuf[0] = 0;
  strcatint(tbuf, "line ", ytru+1);
  strcatint(tbuf, "/", ytot);
  strcatint(tbuf, ", col ", xtru);
  strcatint(tbuf, ", char ",dp-aa);
...
ESTATIC void file_save(void)
{
  ...
  if(flag[CHG] )
  do {
    show_note("Save file (yes/no/cancel): ",0);
    k = toupper(get_key());
    if (k == 'C') return;
  } while(k != 'Y' && k != 'N');
  flag[CHG] = 0;
...
{
  if(*dp == EOL)
    show_note("Invalid Pos.",4);
  else {
    show_note("Mark Set",4);
    mk = dp;
  }
}
...
void editor(void)
{
  ...
  if (sbuf && rbuf && aa && bb) {
	...
	outstr("Unable to allocate memory\nReset unit\n");
  }
  if (sbuf) free(sbuf);
  if (rbuf) free(rbuf);
  if (aa) free(aa);
